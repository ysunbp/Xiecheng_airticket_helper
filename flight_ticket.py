...
def xiecheng(dcity, acity, date, price):
    date_list = []
    date0 = date[0:4] + '-' + date[4:6] + '-' + date[6:8]
    date1 = date[0:4] + '-' + date[4:6] + '-' + str(int(date)+1)[6:8]
    date2 = date[0:4] + '-' + date[4:6] + '-' + str(int(date)-1)[6:8]
    ...
    city = { '阿尔山': 'YIE', '阿克苏': 'AKU', '阿拉善右旗': 'RHT', '阿拉善左旗': 'AXF', '阿勒泰': 'AAT', '阿里': 'NGQ',
            ...
            '香格里拉(迪庆)': 'DIG',
            '襄阳': 'XFN', '兴义': 'ACX', '徐州': 'XUZ', '烟台': 'YNT', '延安': 'ENY', '延吉': 'YNJ', '盐城': 'YNZ',
            '伊春': 'LDS',
    ...
    for i in range(1,2):
        ...
        while flag:
            ...
            table1 = PrettyTable(["  航空公司  ","  航班号  ","  机型  ", "  出发时间及起飞机场  ", "  到达时间及降落机场  ","  准点率  ", "  价格  "])
            table2 = PrettyTable(["  航空公司  ","  航班号  ","  机型  ", "  出发时间及起飞机场  ", "  到达时间及降落机场  ","  准点率  ", "  价格  "])
            # print("123",routeList)
            a = 0
            b = 0
            ...
            for route in routeList:
                if len(route.get('legs')) == 1:
                    ...
                    info['DepartureDate'] = flight.get('departureDate')[-8:-3] + flight.get('departureAirportInfo').get("airportName")
                    info['ArrivalDate'] = flight.get('arrivalDate')[-8:-3] + flight.get('arrivalAirportInfo').get("airportName")
                    info['PunctualityRate'] = flight.get('punctualityRate')
                    info['  价格  '] = int(legs.get('characteristic').get('lowestPrice'))

                    if info['  价格  '] <= int(price):
            ...
            print("以下为低于目标价格的航班")
            print("共"+str(a)+"班航班")
            print(dcity, '------->', acity, date_list[i])
            print(table1.get_string(sortby="  价格  ", reversesort=False))
            print("以下为高于目标价格的航班")
            print("共"+str(b)+"班航班")
            print(dcity, '------->', acity, date_list[i])
            print(table2.get_string(sortby="  价格  ", reversesort=False))

            email_flag = False
            if(a == 0):
                email = input("未找到符合要求的航班，是否设置邮件提醒持续搜索：（是/否）")
                if email == "否":
                    flag = False
                else:
...
if __name__ == "__main__":
    dcity = input('请输入起点（中文，例如“北京”）： ')
    acity = input('请输入终点（中文，例如“上海”）： ')
    date = input('请输入出行日期，例如“20190907”： ')
    price = input('请输入目标价格，例如“500”：')
    xiecheng(dcity, acity, date, price)
    a = input('按任意键退出')
